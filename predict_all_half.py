...
def main():
	...
	n_outputs = N_DEVICES + 1

	with tf.Session() as sess:
		file = "./Models/half" + str(N_HIDDEN) + "hidden_template_" + str(TEMPLATE) + ".ckpt"
		file_meta = file + ".meta"
		...
		predict_train_half = np.zeros((n_train, n_outputs))
		predict_test_half = np.zeros((n_test, n_outputs))
		...
		for line in range(n_train):
			...
			predict_train_half[line, :] = predict

		for line in range(n_test):
			feed = inputs_test[line:line+1, :]

			predict = regression.eval(feed_dict = {input:feed})
			predict_test_half[line, :] = predict

		predict_train = dd.get_all_outputs(predict_train_half, inputs_train)
		predict_train = scaler_out.inverse_transform(predict_train)
		predict_test = dd.get_all_outputs(predict_test_half, inputs_test)
		...
		for i in range(to_check):
			...
			#1idx = sorted_error_train[i] #better
