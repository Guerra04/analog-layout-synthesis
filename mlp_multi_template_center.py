...
def main():
	...
	with tf.name_scope("loss"):
		...
		for i in range(N_DEVICES):
			indexes_xmin[i, 2*i] = 1
		indexes_ymin = np.zeros([N_DEVICES, n_outputs], dtype=np.float32)
		for i in range(N_DEVICES):
			indexes_ymin[i, 1+2*i] = 1

		indexes_xmax = np.zeros([N_DEVICES, n_inputs], dtype=np.float32)
		for i in range(N_DEVICES):
			indexes_xmax[i, 3+5*i] = 1
		indexes_ymax = np.zeros([N_DEVICES, n_inputs], dtype=np.float32)
		for i in range(N_DEVICES):
			indexes_ymax[i, 4+5*i] = 1

		xmin = tf.matmul(indexes_xmin, tf.transpose(regression))
		ymin = tf.matmul(indexes_ymin, tf.transpose(regression))
		xmax = tf.add(xmin, tf.matmul(indexes_xmax, tf.transpose(input)))
		ymax = tf.add(ymin, tf.matmul(indexes_ymax, tf.transpose(input)))

		for i in range(batch_size):
			xImin_aux.append(tf.maximum(xmin[:,i:i+1], tf.transpose(xmin[:,i:i+1])))
			yImin_aux.append(tf.maximum(ymin[:,i:i+1], tf.transpose(ymin[:,i:i+1])))

			xImax_aux.append(tf.minimum(xmax[:,i:i+1], tf.transpose(xmax[:,i:i+1])))
			yImax_aux.append(tf.minimum(ymax[:,i:i+1], tf.transpose(ymax[:,i:i+1])))

		xImin = tf.stack(xImin_aux)
		yImin = tf.stack(yImin_aux)
		xImax = tf.stack(xImax_aux)
		yImax = tf.stack(yImax_aux)

		overlaps = tf.multiply(tf.maximum(tf.subtract(xImax, xImin), 0.0), tf.maximum(tf.subtract(yImax, yImin), 0.0))

		overlap = tf.divide(tf.reduce_sum(overlaps) - tf.reduce_sum(tf.trace(overlaps)), 2.0, name="overlap")
